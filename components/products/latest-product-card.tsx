...
export function LatestProductCard({
  ...
  if (principal) {
    return (
      <div className={cn('min-h-fold flex flex-col relative', className)}>
        <Link href={`/product/${product.id}`} className="size-full flex-1 flex flex-col" prefetch>
          <Image
            priority
            src={productImage}
            alt={product.name}
  ...
  return (
    <div className={cn('relative', className)}>
      <Link href={`/product/${product.id}`} className="block w-full aspect-square" prefetch>
        <Image
          src={productImage}
          alt={product.name}
          width={1000}
